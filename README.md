# 📘 E-commerce Product Recommender — Backend API
## This backend provides a Product Recommendation API that analyzes user interactions (views, clicks, purchases) and generates AI-powered explanations using GeminiAI.
🎯 Objective
- Combine recommendation logic with LLM-powered explanations for users.

📌 Referencing the assignment specification
- Input: Product catalog + user behavior
- Output: Recommended products + LLM-generated explanation (“Why this product?”)
- Optional frontend dashboard showing recommendations


🧠Technical Expectations
- Backend API for recommendations
- Database for products & user interaction
- LLM for explanation text


LLM Usage Guidance:
- Example prompt: “Explain why product X is recommended to this user based on their behavior.”


🏗 System Architecture
High-Level Component Diagram

Uploaded separately in repo under
<img width="1108" height="507" alt="image" src="https://github.com/user-attachments/assets/5f56726b-4eae-4702-900f-b27a8d521e4a" />


🚀 Features
✔ Product Recommendation API
- Suggests products based on user browsing patterns
- Category & tag similarity
- Popularity scoring

✔ LLM-powered Explanations
- Each recommended product includes a short explanation generated by GeminiAI using the file openaiClient.js.



✔ MongoDB Database

Stores:
- Products
- Users
- User interactions (views, clicks, purchases)

✔ Seed Script
- Quickly populate:
- Sample products
- A sample user
- Interaction data


🔧 Setup Instructions

 1️⃣ Install dependencies
 cd Backend
 npm install

 2️⃣ Configure environment variables
  PORT=5000
  MONGO_URI=your_mongodb_connection_string
  GEMINI_API_KEY=your_gemini_api_key

  ⚠️ Do NOT upload .env to GitHub.

  3️⃣ Seed the database
  npm run dev

  This inserts:
  - Sample products
  - A test user
  - Interactions

You will receive a User ID in the terminal output.
Use this ID to test the recommendation API.

4️⃣ Start the backend server

npm run dev

📡 API Endpoints

### GET /api/recommend/:userId
Fetch personalized recommendations and AI explanations.

Example Request:
[http://localhost:5000/api/recommend/user123]

Example Response:
{
  "user": {
    "id": "65f12abc...",
    "name": "Test User"
  },
  "recommendations": [
    {
      "product": {
        "title": "Nike Air Max",
        "category": "Shoes",
        "price": 6999
      },
      "explanation": "Recommended because you often view sports and running products."
    }
  ]
}


🧠 How Recommendations Work
Logic used (services/recommendService.js):
- Category matching
- Tag similarity (Jaccard similarity score)
- Popularity boost
- Sorted by score
- Top 5–6 items returned

Explanation generation is done through:
services/llmservices.js → GeminiAI model.


🧪 Testing the API

Test in Postman:
- Method: GET
- URL: [http://localhost:5000/api/recommend/user123]

🛡️ Security Notes
- Never expose your .env file
- Use MongoDB user credentials, not admin user
- Restrict GeminiAI key usage to server-side only


📌 Future Improvements (optional)
- Collaborative filtering
- Embedding-based product similarity
- JWT authentication
- Caching LLM explanations
- Admin panel for products

👤 Author & Contact
Rakesh Kumar
9646832910 mail- rakesh9803646587@gmail.com
